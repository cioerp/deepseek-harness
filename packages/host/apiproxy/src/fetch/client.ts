...
export abstract class AbstractApiClient implements IApiClient {
  ...
  protected mintRpcId(): RpcId {
    // crypto.randomUUID is a Web API (browser + Node ≥19): keeps this base
    // platform-neutral, but browsers expose it only in secure contexts (HTTPS
    // or loopback); randomUuid falls back to crypto.getRandomValues so a
    // plain-HTTP LAN deployment keeps correlating its RPCs.
